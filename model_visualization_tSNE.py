...
class SequenceDataset(Dataset):  # type: ignore[type-arg]
    ...
    def __init__(self, sequences: List[str], seq_length: int, tokenizer: PreTrainedTokenizerFast, kmer_size: int = 3, verbose: bool = True,):
        self.batch_encodings = self.tokenize_sequences(sequences, tokenizer, seq_length, kmer_size, verbose)

    @staticmethod
    def tokenize_sequences(sequences: List[str], tokenizer: PreTrainedTokenizerFast, seq_length: int, kmer_size: int = 3,verbose: bool = True,) -> List[BatchEncoding]:

        tokenizer_fn = functools.partial(tokenizer, max_length=seq_length, padding="max_length", truncation=True, return_tensors="pt",)

        batch_encodings = [tokenizer_fn(SequenceDataset.group_and_contextualize(seq, kmer_size))for seq in tqdm(sequences, desc="Tokenizing...", disable=not verbose)]
...
tokenizer = get_tokenizer(sequences, tokenizer_checkpoint=tokenizer_checkpoint, vocab_size=50_257)

# TODO: see how to get the seq_length from the args in main_llm:
seq_length = 1024
device = "cuda" if torch.cuda.is_available() else "cpu"

model = get_model(tokenizer=tokenizer, model_architecture='bert_3m', model_checkpoint=None)
model.eval()
...
embedding_visualization = PlotClustersData(tsne_embeddings, label_array, gc_content_of_seqs, label_dict)

# plotting all clusters colored with label

plot_df_separate, hue_separate, plt_title = embedding_visualization.separate_clusters_labels()
...
for x in range(len(label_categories)):
  plot_df_separate_gc_coding, hue, plt_title = embedding_visualization.separate_clusters_gc_content(label_mask=x)
...
plot_df_separate, hue_separate, plt_title = embedding_visualization.plot_both_clusters_gc_content()
embedding_visualization.plot_clusters(plot_df_separate, hue_separate, plt_title)
plt.show()


X_train, X_test, y_train, y_test = train_test_split(embeddings, labels, stratify=labels, random_state=1)
