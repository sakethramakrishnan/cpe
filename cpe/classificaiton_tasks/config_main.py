...
from utils import save_ckp, load_ckp, load_most_recent_checkpoint, train_biological_tokenizer
...
def train_model(model, task_type, train_generator, valid_generator, test_generator, epochs, results_path, start_epoch, config, optimizer):
    ...
    def calc_metrics_regression(model, generator):
        ...
        with torch.no_grad():
            ...
            y_pred = torch.tensor(y_pred).squeeze() # because it is in size (n, 1), we want it in size (n)
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        model.train()
        for idx, (x, y) in tqdm(enumerate(train_generator)):
            outputs = model(ids=x['ids'], mask=x['attention_mask'], token_type_ids=x['token_type_ids'])
            print(outputs.shape)
...
def main():
    ...
    model = BioBERTModel(config.hidden_size, config.num_layers, config.num_attention_heads, num_classes, max_position_embeddings=config.max_length)
