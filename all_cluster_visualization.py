...
class PlotClustersData:
  ...
  def __init__(self, tsne_hidden_states: npt.ArrayLike, labels: npt.ArrayLike, gc_content: npt.ArrayLike, label_dict: Dict):
  ...
  def separate_clusters_gc_content(self, label_mask: int):
    ...
    hue = new_gc
    type_of_seq = list(self.label_dict.keys())[list(self.label_dict.values()).index(label_mask)]
  ...
  def plot_both_clusters_gc_content(self):
    ...
    plot_df = pd.DataFrame({"x": self.tsne_hidden_states[:, 0], "y": self.tsne_hidden_states[:, 1]})
